...
export class Context {
  ...
  constructor(config: ContextConfig = {}) {
    ...
    console.log(
      `🔧 Initialized with ${this.supportedExtensions.length} supported extensions and ${this.ignorePatterns.length} ignore patterns`
    );
  ...
  async indexCodebase(
    ...
    console.log(
      `✅ Codebase indexing completed! Processed ${result.processedFiles} files in total, generated ${result.totalChunks} code chunks`
    );
  ...
  async reindexByChange(
    ...
    console.log(
      `🔄 Found changes: ${added.length} added, ${removed.length} removed, ${modified.length} modified.`
    );
    ...
    console.log(
      `✅ Re-indexing complete. Added: ${added.length}, Removed: ${removed.length}, Modified: ${modified.length}`
    );
    progressCallback?.({
  ...
  private async deleteFileChunks(
    ...
    // Escape backslashes for vector database query expression (Windows path compatibility)
  ...
  async semanticSearch(
    ...
    if (isHybrid === true) {
      try {
        ...
        console.log(
          `🔍 Collection '${collectionName}' exists and appears to have data`
        );
      } catch (error) {
        console.log(
          `⚠️  Collection '${collectionName}' exists but may be empty or not properly indexed:`,
          error
        );
      }
      ...
      console.log(
        `🔍 First 5 embedding values: [${queryEmbedding.vector
          .slice(0, 5)
          .join(", ")}]`
      );
      ...
      console.log(
        `🔍 Search request 1 (dense): anns_field="${searchRequests[0].anns_field}", vector_dim=${queryEmbedding.vector.length}, limit=${searchRequests[0].limit}`
      );
      console.log(
        `🔍 Search request 2 (sparse): anns_field="${searchRequests[1].anns_field}", query_text="${query}", limit=${searchRequests[1].limit}`
      );
      ...
      if (results.length > 0) {
        console.log(
          `🔍 Top result score: ${results[0].score}, path: ${results[0].relativePath}`
        );
      }
  ...
  async clearIndex(
    ...
    const collectionName = this.getCollectionName(codebasePath);
    const collectionExists = await this.vectorDatabase.hasCollection(
      collectionName
    );
  ...
  addCustomIgnorePatterns(customPatterns: string[]): void {
    ...
    this.ignorePatterns = uniquePatterns;
    console.log(
      `🚫 Added ${customPatterns.length} custom ignore patterns. Total: ${this.ignorePatterns.length} patterns`
    );
  }
  ...
  resetIgnorePatternsToDefaults(): void {
    ...
    console.log(
      `🔄 Reset ignore patterns to defaults: ${this.ignorePatterns.length} patterns`
    );
  }
  ...
  private async prepareCollection(
    ...
    if (collectionExists && forceReindex) {
      console.log(
        `🗑️  Dropping existing collection ${collectionName} for force reindex...`
      );
      await this.vectorDatabase.dropCollection(collectionName);
    ...
    console.log(
      `🔍 Detecting embedding dimension for ${this.embedding.getProvider()} provider...`
    );
    const dimension = await this.embedding.detectDimension();
    console.log(
      `📏 Detected dimension: ${dimension} for ${this.embedding.getProvider()}`
    );
    ...
    console.log(
      `✅ Collection ${collectionName} created successfully (dimension: ${dimension})`
    );
  }
  ...
  private async processFileList(
    ...
    for (let i = 0; i < filePaths.length; i++) {
      ...
      try {
        ...
        if (chunks.length > 50) {
          ...
          console.log(
            `📄 Large file ${filePath}: ${Math.round(
              content.length / 1024
            )}KB -> ${chunks.length} chunks`
          );
        }
        ...
        for (const chunk of chunks) {
          ...
          if (chunkBuffer.length >= EMBEDDING_BATCH_SIZE) {
            try {
              ...
              console.error(
                `❌ Failed to process chunk batch for ${searchType}:`,
                error
              );
          ...
          if (totalChunks >= CHUNK_LIMIT) {
            console.warn(
              `⚠️  Chunk limit of ${CHUNK_LIMIT} reached. Stopping indexing.`
            );
            limitReached = true;
    ...
    if (chunkBuffer.length > 0) {
      ...
      try {
        ...
        console.error(
          `❌ Failed to process final chunk batch for ${searchType}:`,
          error
        );
        if (error instanceof Error) {
          console.error("Stack trace:", error.stack);
  ...
  private async processChunkBuffer(
    ...
    const searchType = isHybrid === true ? "hybrid" : "regular";
    console.log(
      `🔄 Processing batch of ${chunks.length} chunks (~${estimatedTokens} tokens) for ${searchType}`
    );
  ...
  private async findIgnoreFiles(codebasePath: string): Promise<string[]> {
    try {
      ...
      if (ignoreFiles.length > 0) {
        console.log(
          `📄 Found ignore files: ${ignoreFiles
  ...
  private async loadIgnoreFile(
    ...
    try {
      ...
      if (ignorePatterns.length > 0) {
        console.log(
          `🚫 Loaded ${ignorePatterns.length} ignore patterns from ${fileName}`
        );
  ...
  addCustomExtensions(customExtensions: string[]): void {
    ...
    console.log(
      `📎 Added ${customExtensions.length} custom extensions. Total: ${this.supportedExtensions.length} extensions`
    );
  }
