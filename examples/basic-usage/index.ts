...
import { envManager } from "@codesearch/core";
...
async function main() {
  ...
  try {
    ...
    // Set to true to use RESTful API (for environments without direct client support)
    // Set to false to use direct client (default, more efficient)
    ...
    const splitterType =
      envManager.get("SPLITTER_TYPE")?.toLowerCase() || "ast";

    console.log(
      `🔧 Using ${
        useRestfulApi ? "RESTful API" : "direct client"
      } implementation`
    );
    ...
    let vectorDatabase;
    if (useRestfulApi) {
      // Use RESTful implementation (for environments without direct client support)
      vectorDatabase = new QdrantRestfulVectorDatabase({
        url: qdrantUrl,
        ...(qdrantApiKey && { apiKey: qdrantApiKey }),
      });
    } else {
      // Use direct client implementation (default, more efficient)
    ...
    for (const query of queries) {
      ...
      if (results.length > 0) {
        results.forEach((result, index) => {
          console.log(
            `   ${index + 1}. Similarity: ${(result.score * 100).toFixed(2)}%`
          );
          console.log(
            `      File: ${path.join(codebasePath, result.relativePath)}`
          );
    ...
    if (error instanceof Error) {
      if (error.message.includes("API key")) {
        ...
        console.log("\n💡 Please make sure Qdrant service is running");
        console.log("   - Default URL: http://localhost:6333");
        console.log("   - Can be modified via QDRANT_URL environment variable");
        console.log("   - For RESTful API: set useRestfulApi=true");
        console.log(
          "   - For direct client (default): set useRestfulApi=false or leave unset"
        );
