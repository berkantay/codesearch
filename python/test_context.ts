...
import { QdrantVectorDatabase } from "../packages/core/src/vectordb/qdrant-vectordb";
...
export async function testContextEndToEnd(config: {
  ...
  try {
    ...
    console.log("🗄️ Creating Qdrant vector database instance...");
    ...
    if (!hasIndex) {
      ...
      indexStats = await context.indexCodebase(
        config.codebasePath,
        (progress) => {
          console.log(
